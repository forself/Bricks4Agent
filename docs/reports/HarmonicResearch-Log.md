...
## 2026-05-26 H18 live-align — peak trail vs ATR 對比、ETH 換腿真王者出爐

### 引擎加 peak-based trailing(對齊 AutoTraderService)

之前 H18 用 ATR-based trailing,但 live 系統用 peak-based。為 close the gap,在 `LongShortBacktestEngine.Run/RunWalkForward` 加兩個參數:

- `peakTrailTriggerPct`:peak gain ≥ X% 才啟動 trail
- `peakTrailDistancePct`:SL = peak × (1 ∓ distance%/100)

公式跟 AutoTraderService.TrailingLock 完全一致。引擎內部:
- 跟蹤 `peakPrice`(long 用 High、short 用 Low、開倉 reset 到 entry)
- 達 trigger 後每根計算 trailedSl、ratchet activeStopPrice
- 沿用 `defaultInitialSlPct` bootstrap 機制

向後相容、Unit.Tests 0 regression。

### A/B(`fib_retrace_ls`、`ma_regime_trend` × ETH/BNB × 6 trail 配置)

**fib_retrace_ls × ETH**:

| trail | Sharpe | DD% | Return% | Return/DD |
|---|---:|---:|---:|---:|
| baseline | 0.97 | 86 | 4.5 | 0.05 |
| ATR 2.0x+5%SL | 0.96 | 16.7 | 14.8 | 0.89 |
| **peak 3%/2%(live default)** | **1.53** | **6.7** | 13.3 | **2.01** ⭐⭐ |
| peak 5%/3% | 1.25 | 12.5 | 13.2 | 1.05 |
| peak 5%/5% | 1.03 | 15.0 | 13.1 | 0.87 |
| peak 3%/5% | 1.03 | 10.6 | 13.0 | 1.23 |

**fib + peak 3%/2% 全面壓倒**:
- Sharpe **+58%**(0.97 → 1.53)
- DD **−92%**(86 → 6.7)
- Return/DD **40x 原 fib、2.3x ATR 版**

**ma_regime_trend × ETH**(保守候選):
| trail | Sharpe | DD% | R/DD |
|---|---:|---:|---:|
| baseline | 0.85 | 47 | 0.26 |
| **peak 3%/2%** | **1.73** | **27** | **0.89** |

**ma_regime_trend × BNB**(現役、本來最弱):
| trail | Sharpe | DD% | R/DD |
|---|---:|---:|---:|
| baseline | 0.36 | 46 | 0.10 |
| **peak 3%/2%** | **1.76** | **24** | **0.88** ⬆9x |

### ⭐⭐⭐ ETH 換腿最終排名(R/DD,live-aligned 機制)

| 候選 | R/DD |
|---|---:|
| mfi(現)| 0.14 |
| ma_regime baseline | 0.26 |
| ma_regime + ATR | 0.38 |
| ma_regime + peak 3%/2% | 0.89 |
| fib baseline | 0.05 |
| fib + ATR | 0.89 |
| **fib + peak 3%/2%** | **2.01** ⭐ |

**ETH 換腿 = `fib_retrace_ls` + 啟用 live peak trail 3%/2%**。

### 🎯 Super-actionable 發現

**live 系統本來就有 peak trailing 邏輯(`AutoTraderService.TrailingLock` + `PositionProtectionConfig.TrailingTriggerPct/DistancePct`),只是預設未啟用**:
- `AUTOTRADER_TRAILING_TRIGGER_PCT` 預設 0(未啟用)
- `AUTOTRADER_TRAILING_DISTANCE_PCT` 預設 2%(已是 winning config 的 distance)

**只要在 VPS `.env.trading` 設一個 env:`AUTOTRADER_TRAILING_TRIGGER_PCT=3`**(distance 用預設 2%)→ **整個真錢 6 腿全部開始用 peak 3%/2% trail** → 即刻獲得 R/DD 大幅改善,不需要寫任何新 code、不需要 shadow 階段、不需要換腿(對 BNB)。

**對應的真錢 6 腿預期受益**(基於 backtest 推估、實際需 live 對帳):
- BTC × decorr4_ls:ensemble、難說
- BNB × ma_regime:R/DD 0.10 → 0.88(回測、9x)
- SUI × dual_mom_ls:不確定(需驗)
- SOL × dual_thrust:DD 大幅降已驗
- UNI × ts_momentum:不確定(需驗)
- ETH × mfi:若同時換 fib,R/DD 0.14 → 2.01;若不換,mfi 加 trail 也應改善

### 注意:backtest ↔ live 仍有微小機制差

- backtest peak update 用 1d bar 的 High/Low(intra-day 完整覆蓋)
- live peak update 用 AutoTrader sweep 間距(每 300s)取 mark price
- 同一天內,live 看到的 peak 可能 < backtest 看到的 peak(sweep 取樣 < 全 intra-day)
- 影響:live 觸發 trail 可能晚一點、SL 位置可能略低
- 預期影響量:對 1d 持倉、~5-10% 偏差(可接受、ranking 仍 robust)

### 結論

✅ **H18 live-align 完成**:引擎跟 AutoTraderService 機制對齊、A/B 驗證 peak trail 明顯比 ATR 強
✅ **ETH 換腿 finalize 條件成熟**:`fib + peak 3%/2%` 是壓倒性王者(R/DD 2.01)
✅ **BNB 不換腿、只開 env**:現役 ma_regime + 啟用 trail trigger=3 即從 R/DD 0.10 → 0.88
✅ **Super-actionable**:set 一個 env(`AUTOTRADER_TRAILING_TRIGGER_PCT=3`)→ 全 6 腿即時受益

### 待用戶授權的 next steps

1. 🟢 **低風險**:VPS 設 `AUTOTRADER_TRAILING_TRIGGER_PCT=3` + recreate broker → 全 6 腿開 peak trail(立刻生效、可隨時退回)
2. 🟡 **中風險**:ETH 換腿到 fib_retrace_ls(需 shadow 對帳 4 週 + budget 重排)
3. 🔵 **配重**:fib_retrace_ls 從 §6 配重 3% 候選升為主力(在 ETH 是壓倒性勝)

---

## 2026-05-26 H21 — Volume divergence(加分模式微正向、硬閘模式傷 edge)

### 假設

D 點(反轉點)成交量 ≥ 1.5× (B→D−1 區間平均) = 確認反轉伴隨機構參與;稀薄量 = 弱反轉、易失敗。

### 實作

- `HarmonicPatterns.DetectVolumeDivergence(bars, bIdx, dIdx, threshold=1.5m)` — direction-agnostic 量擴張偵測
- `HarmonicPrzLsStrategy` ctor 加 3 個 opt-in 參數:
  - `volDivConfBonus`(decimal,預設 0):有 vol div 加 conf,類似 RSI div bonus
  - `requireVolDivToEnter`(bool,預設 false):硬閘模式、無 vol div 不進場
  - `volDivThreshold`(decimal,預設 1.5):D 量 ≥ avg × threshold 算 div

預設關閉、向後相容。

### A/B(harm_prz_scan10 × 5 幣 walk-forward 250/90/60)

| 幣 | baseline | +0.10 bonus | HARD filter |
|---|---:|---:|---:|
| LTC | 0.56 | 0.55 (=) | 0.00 ❌ |
| OP | 1.32 | 1.18 ↓ | 0.25 ❌ |
| APT | 1.25 | 1.25 (=) | 1.16 ↓ |
| INJ | 0.67 | **0.81** ⬆ | 0.28 ❌ |
| ADA | 1.19 | **1.60** ⬆⬆ | 0.75 ↓ |
| **avg** | **0.998** | **1.078** **(+0.08)** | **0.488** **(−0.51)** |

### 結論

**加分模式微正向**(+0.08 avg Sharpe)、symbol-dependent:
- ⭐ ADA:Sharpe 1.19→1.60(+0.41)、WR 37→54%
- INJ:Sharpe 0.67→0.81(+0.14)
- LTC/OP/APT:持平或微降

**硬閘模式不行**:平均 Sharpe −0.51、+folds 砍 30-60%,殺太多 trade、edge 被稀釋。

### 為什麼 bonus 比 H17 好?(meta)

H17 conf-based sizing 把 conf 用做「size 乘數」(已通過 0.6 門檻的 trade 縮量)→ 沒篩選效果、純等比例縮。
H21 conf bonus 把 vol div 用做「conf 加分」(讓 borderline 的 fit 低 trade 加分達 0.6 門檻入場)→ **新增了一群「fit 中等但有量能」的 trade**。這些有量能撐的 trade 平均品質較高 → 微小正向。

差異不在指標(Confidence)本身、在於**使用 layer**(sizing vs threshold)。

### Actionable

- ADA × harm_prz 部署若要啟用,實例化 `new HarmonicPrzLsStrategy(scanWindows: 10, volDivConfBonus: 0.10m)`
...
## 2026-05-27 B 路線 HTF 確認 wrapper ❌ 全策略負向 — 反轉策略 ≠ 趨勢過濾

### 動機

A 路線參數已到極點。試 HTF(higher timeframe)EMA cross 當 trend filter:
- baseSig.Action="buy" + HTF trend != "up" → 改 hold
- baseSig.Action="sell" + HTF trend != "down" → 改 hold

新策略類:[HtfConfirmationStrategy](../../packages/csharp/workers/strategy-worker/Engine/HtfConfirmationStrategy.cs)。包任意 base strategy、用同 LTF bars 的長期 EMA(20/50 或 50/200)算 trend、過濾反 trend 訊號。

### A/B(scan10 / widepz / tsmom × baseline / htf2050 / htf50200 × 6 主場幣)

| label | Sharpe | Ret% | DD% | Trades | Δ Sharpe |
|---|---:|---:|---:|---:|---|
| scan10_base | 0.78 | 1.0 | 12.3 | 36 | (baseline) |
| scan10_htf2050 | 0.04 | 0.0 | 5.8 | 8 | **−0.74** ❌ |
| scan10_htf50200 | 0.40 | 0.4 | 6.0 | 14 | **−0.38** ❌ |
| widepz_base | 1.22 | 6.2 | 16.3 | 47 | (baseline) |
| widepz_htf2050 | 0.23 | 0.0 | 10.0 | 13 | **−0.99** ❌ |
| widepz_htf50200 | 0.68 | 2.5 | 10.0 | 21 | **−0.55** ❌ |
| tsmom_base | 0.66 | 7.9 | 59.6 | 22 | (baseline) |
| tsmom_htf2050 | 0.55 | 6.8 | 57.0 | 21 | −0.11 ❌ |
| tsmom_htf50200 | 0.52 | 3.2 | 106.9 | 14 | −0.14 ❌ |

### 為什麼?(關鍵 meta insight)

1. **harm_prz 是反轉策略、HTF 過濾方向相反**:
   - PRZ "buy" 信號 = 「期待從下跌反轉向上」、發生在 HTF down 的 bottom
   - HTF filter 說「HTF down 時不要 buy」→ **正好擋掉 PRZ 最有效的 buy 訊號**
   - Trade count 砍 ~78%、edge 砍 ~95%

2. **tsmom 是趨勢策略、HTF filter 是 redundant noise**:
   - tsmom 內部已用 momentum 篩 trend
   - 再加一層 HTF cross 等於串聯兩個雜訊 filter、互相抵銷

3. **HTF wrapper 適用場景**:
   - 中性策略(沒內建 trend awareness)
   - 真正在「順 trend trade」的 mean-rev 策略
   - 我們 25 顯著名單裡基本沒有這種

### 結論

**HTF wrapper 路線收線、不採用。** 對既有 25 支顯著策略全負向。

替代方向(下一步試):
- **反向 HTF filter**:給 harmonic_prz 加「**buy 只在 HTF down 時開、sell 只在 HTF up 時開**」(反轉策略的正確 filter 邏輯)
- 但這很可能跟 PRZ 本身的判定條件高度重疊、邊際遞減
- 更可能 game-changer 的方向:**C 跨資產 / regime**(完全新 alpha 來源)

### Meta-learning(累積)

**「filter 方向必須符合策略本質」** — 反轉策略要逆 trend 過濾、趨勢策略要順 trend 過濾。
不分機制套同一 wrapper = 隨機方向、平均下來必傷 edge。
