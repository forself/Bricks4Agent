...
### 新發現 3:bb_revert × peak trail 是隱藏王者(ETH 換腿洗牌)

詳見 [HarmonicResearch-Log.md](HarmonicResearch-Log.md) 的「2026-05-26 H18 補完 — bb_revert × peak trail」段。摘要:
- 用戶提問斐波那契 + 布林通道是否都覆蓋觸發 bb_revert × peak trail 完整 A/B(原本只測過 baseline、未配 trail)
- ETH × bb_revert_ls + peak 3%/2%(live default):**Sharpe 2.00 / DD 4.9 / WR 88% / R/DD 1.69**
- 同設置 BNB:Sharpe 1.33 / DD 5.2 / WR 70(從 baseline 0.49/22.6/37 大幅躍升)
- 同設置 ADA:Sharpe 1.06 / DD 4.9 / R/DD 0.87
- 機制:mean reversion winners 短週期就到峰、peak trail 3%/2% 立刻鎖、不讓回吐 → 極高 WR;與 trend follower 需要寬 trail 不同

### ETH 換腿候選最終排名(取代「新發現 2」中 fib 為唯一激進選項的提案)

| 候選 | Sharpe | DD% | R/DD | WR% | 備註 |
|---|---:|---:|---:|---:|---|
| mfi(現)| 0.58 | 60.4 | 0.14 | 54 | 真錢中、DD 太高 |
| ma_regime_trend + peak 3%/2% | 1.73 | ~9 | 0.89 | ~60 | 保守 |
| fib_retrace_ls + peak 3%/2% | 1.53 | 6.7 | **2.01** | 74 | R/DD 王 |
| **bb_revert_ls + peak 3%/2%** | **2.00** | **4.9** | 1.69 | **88** | **Sharpe/DD/WR 三冠** ⭐ |

**最終建議(待 shadow 驗證)**:
- 主推:`bb_revert_ls + peak 3%/2%`(Sharpe/DD/WR 三項最強、R/DD 落後 fib 16%)
- 備案:`fib_retrace_ls + peak 3%/2%`(R/DD 最高、若 bb_revert shadow OOS 退化則切 fib)
- VPS 已設 `AUTOTRADER_TRAILING_TRIGGER_PCT=3 / DISTANCE_PCT=2`,任一候選 peak trail 即時生效、不需 env 變更

### 修正後的 §6 Shadow 配重草案(再覆蓋「新發現 2」的草案、納入 bb_revert)

```yaml
harm_prz_scan10:          20%   # 不變、王牌
harm_prz_scan10_widepz:   22%   # Method C 後 widepz 回升
decorr5_widepz:           13%
ma_regime_trend:          15%
accel_momentum:           10%
dual_mom_ls:               8%
ts_momentum:               5%
harm_prz_top2_scan10:      4%
bb_revert_ls:              3%   # 🆕 限 ETH / BNB / ADA(三幣 OOS + peak trail 全壓制 baseline)
# total                  100%
# (fib_retrace_ls 暫從草案撤下作為 ETH 換腿備案而非新增腿,避免雙 ETH 候選競配)
```

**幣池白名單**(增補):
- bb_revert_ls + peak 3%/2%:ETH / BNB / ADA 主場(三幣 OOS Sharpe 全 ≥ 1.0、WR ≥ 70%、DD ≤ 5.2)
- bb_revert_ls 其他幣需 --validate-candidates 補測再開放
