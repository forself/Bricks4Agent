...
### 14.8 Dashboard MVP-3：把 14.7 的執行細節變成可看（2026-05-02）

**問題**：14.7 跑完後 inbox 已經有 tool calling 紀錄，但 dashboard 上看到的只有 reply 字串本身的「🔧 工具呼叫（N 次）」附加 trace（commit 17c44b8）——**結構化資料埋在文字裡、無法 filter / 無法切 agent 觀察 / 無法看 LLM 那層原始 tool call args**。Research Assistant 這條 demo 路徑做完了，但沒有「秀出來」的介面。

**做法**：

1. **Dispatch dialog 多 agent 切換**（commit 702dcad）  
   原本派任務對話框綁死單一 agent，要看別的 agent 任務歷史只能關閉再點。改成 dropdown 直接切，dialog 內既有的 5 秒 inbox poll 一直讀 `dispatchAgent.value`，所以中途切 agent 自動跟上。新增 4 欄摘要面板（Principal / Role / TaskType / State），切換時看得到差異。
2. **LLM Proxy 結構化 tool calling 觀測**（commit 3aff8f0）  
   - `LlmCallRecord` schema 加 `ToolCalls: List<{Name, ArgsBrief}>`（args 截 200 字避免 ring buffer 爆量）
   - `MeteredLlmProxyService.ChatAsync` 成功路徑解析 `result.ToolCalls` JsonArray、寫入 record
   - `/api/v1/llm-proxy/recent` 每筆 entry 多一欄 `tool_calls`
   - Dashboard 表加可展開列：Tools 欄顯示 `🔧 N` badge、▶ 三角展開看每個 tool 的 name + args JSON
   - **重要設計分界**：metering 層只記 LLM「要求調用」的 tool calls，不記執行結果——執行細節是 agent runtime 的職責、已經在 reply text trace 裡（commit 17c44b8）。兩層各自負責、不互相覆蓋
3. **Element Plus 渲染 bug 順手修**（commit bed859a + 62e4df7）  
   debug 過程意外發現一個既有 pattern bug：`<el-card><el-empty v-if/><el-table v-else/></el-card>` 在 Vue 3.5.13 + Element Plus 2.9.1 環境下整個 default-slot subtree 不掛上（兩個分支都不渲染、body 只剩 padding 高）。修法是把 `el-empty + el-table` 收成**單一 el-table 用內建 `:empty-text`**。順手修了 LLM Proxy 兩張卡 + 系統診斷 issue 卡——後者本來表結構就寫好了、只是因為這個 bug 從來沒亮過
4. **系統診斷 issue 分組顯示**（commit 62e4df7）  
   原本 31 筆 issue 一字排開、`Connection closed by broker` 重複 6+ 次。改成按 `(severity, category, entity_id, message)` 收攏成 group，每 group 一列加 `×N` 次數 badge，▶ 展開看所有發生時間。預設排序：critical 先 → 同 severity 內次數高的先。加 chip 篩選條（全部/嚴重/錯誤/警告）
5. **Robustness fix：list 不再悄悄消失**（commit f9efb3c）  
   `loadAgents` / `loadCaps` 原本 catch 直接 `agents.value=[]`。重整時 ECDH handshake race 或 broker 重啟 invalidates session，導致暫時性 fail → list 被清空 → 視覺上等於「broker 沒 agent」。改成保留上一次值 + console.error 印失敗原因，使用者真的需要看時還是有資料

**這層補完了什麼**：

> 14.7 把治理鏈跑通，14.8 把跑通的東西**從「DB 紀錄 / log」變成「儀表板上看得到的結構化視圖」**。多 agent 切換代表平台上同時有多個 agent 是 first-class scenario；LLM Proxy tool call 細節讓「LLM 要求調用什麼」跟「實際執行結果是什麼」分兩層觀察成為可能（前者在 LLM Proxy 分頁、後者在 agent task history）；診斷分組讓 ops 不用滑半天找根因。**14.7 是「能跑」的證據、14.8 是「能維運」的證據。**

---

### 14.10 Auto-Trader 收尾：從「能跑」到「能跑在生產線」（2026-05-02）

**問題**：Auto-Trader 主迴圈（quote → strategy → risk → order）整條跑得通，但 6 個月沒動的實作有一堆生產級缺口——稽核掃下來像這樣：watchlist 只在記憶體（broker 重啟全消失）、order 沒 idempotency key（重試 retry 會送重複單）、daily_trade_count 永遠傳 0（風控規則形同虛設）、下完單沒人追蹤成交（fill 從不回流到本地 DB）、零個 e2e 測試、風控規則也少了現金底線、新增持倉上限、跨單的反抖動 cooldown、交易時段限制這幾條常見規則。

**做法**（4 個 cut + 4 條新風控規則）：

1. **Cut 1：watchlist 持久化**（commit 182d2f3）  
   `AutoTraderService._watchList` 從 `ConcurrentDictionary` 搬到新表 `auto_trade_watchlist`，constructor 從 DB load、Add/Remove/Pause/Resume/poll 後的 last_signal 全寫回。broker 重啟後 watchlist 完整保留。
2. **Cut 2a：Order idempotency**（commit 0ba7dd0）  
   `TradingOrder.OrderId` 接受呼叫者帶來的 `client_order_id`，trading-worker handler 在打交易所前先查 DB 看這個 ID 已有非 rejected 紀錄、有就直接回那筆（DB 層 dedup）。`AlpacaClient` / `BinanceClient` 把 OrderId 當 `client_order_id` / `newClientOrderId` 送上交易所（exchange 層 dedup，雙保險）。AutoTrader 用 `(exchange, symbol, side, qty, epoch5min)` 算 deterministic key，5 分鐘 bucket 內 retry 同樣意圖永遠收到同一個 ID、跨 bucket 是新意圖。Alpaca paper trading e2e 實測：1st 建單拿到 `external_id=53b27f31`、2nd 同 key 直接拿回相同 external_id（沒再打 Alpaca）、3rd 不同 key 才建新單。
3. **Cut 2b：Fill polling + daily count**（commit df2e737）  
   trading-worker 多了個 `FillPollerService` 背景跑，30 秒一輪把 non-terminal 訂單從交易所抓回最新狀態同步到本地 orders 表，狀態變 `filled` 時用 `trade_id="fill-{externalId}"` idempotent 寫一筆 trade 紀錄（重 poll 不會重複插入）。新增 `GetDailyTradeCount(fromUtc)` 查詢給 risk engine 餵 `max_daily_trades` 規則，AutoTrader 在風控前先 dispatch `trading.account / daily_trade_count` 拿真實數字。原本永遠是 0 的計數器終於有用。
4. **Cut 3 + Cut 4：xUnit 回歸網**（commits 8a2a13b、92e32e7）  
   broker 端 `AutoTraderServiceTests` 12 個 case 鎖 watchlist persistence + idempotency key 契約；trading-worker 端開新 `TradingWorker.Tests` project（packages/csharp/tests/trading-worker-tests/）包 13 個測試覆蓋 `FillPollerService` 各種 race / 例外路徑跟兩個 DB 查詢。整體 xUnit 92 → 127（+35），TradingWorker.Tests 0 → 13。

**4 條新風控規則**（commits 2ea4366、613f415、0451843）：

| 規則 | 解的什麼問題 | 預設 |
|---|---|---|
| `min_cash_reserve` | 防 buy 把現金梭哈光、留底給滑價 / 手續費 / 緊急出場 | $500 floor |
| `max_position_count` | 限制同時持有的不同標的數，防 strategy 失控亂開倉 | 10 symbols |
| `cooldown_seconds` | 同 (exchange,symbol) 兩次成交之間的最短間隔，防 signal 抖動連續 buy/sell flapping | 60s |
| `time_window` | 只允許在指定 UTC 時段下單，避開美股開盤 / 收盤 15 分鐘高波動 | disabled (24/7 crypto 預設不擋) |

`time_window` 帶出 `RiskRule.Params: string?` 這個新欄位——之前所有規則都吃單一 decimal threshold 不夠表達「視窗起迄」這種雙值參數。Params 是 JSON、各 rule type 自己定義 schema、解析失敗 silently 不擋（避免設定錯就把交易擋光）。**這條設計開了未來規則擴充的 generic slot**——下一條要加「波動率上限」/「sector exposure」之類複雜規則時不用再動 RiskRule schema。

**這層補完了什麼**：

> Auto-trader 的「能跑」（survey 當下整鏈通過）跟「能跑在真實帳戶」（重啟存活、retry 不亂、fill 追得到、風控餵真實數字、有測試鎖契約）是兩件事。14.10 是把後者實證出來——10 條風控規則 + 4 條防呆機制 + 13+12 個 xUnit 鎖住核心契約 + Alpaca paper trading e2e 跑通 idempotency。**這也驗證了一個架構判斷**：把「control plane（broker / risk-worker）」跟「execution plane（trading-worker）」用 worker SDK 切開、用 capability route 而不是直接呼叫，當需要在中間插一層（fill poller、idempotency dedup、daily count）時不用動 broker 核心、只在執行層加新 service。**§五的關卡乾淨切開、新工程繞著它建**這個直覺，這次驗證得最徹底。

---

### 14.11 整輪工程的小結（給說詞用）

九個小節合起來把專題主軸文件的抽象主張**全部變成有 demo 證據的實證**：
...
| §三 可觀測 / 可維運（治理層的事故能被看到、不只是被記） | 14.8（dashboard 多 agent 切換 + tool call 結構化視圖 + issue 分組） |
| §五 control plane / execution plane 切分讓擴充不動核心 | 14.10（auto-trader 加 fill poller / idempotency / 4 條風控規則完全沒動 broker） |
...
## 十五、可觀測 + 治理層完整閉環（2026-05-09）

§14 把儀表板從「能看 worker」推到「能追 agent 任務」、補完了 LLM proxy / 容器日誌 / 錯誤分類三組觀測點。§15 則進一步把**派發層本身**（IExecutionDispatcher → PoolDispatcher）加上完整的稽核 + 治理 + 可觀測閉環，讓「dashboard 直呼 dispatcher 的輕量路徑」也享有跟 BrokerService 16-step PEP 同等的審計強度。一輪 12 個 commit、完全 additive、沒動既有 worker 介面。

### 15.1 IStrategyRegistry——策略介面 self-describing 化

**問題**：StrategySignalHandler 早期把 13 個策略塞在 `Dictionary<string, IStrategy>` 裡、靠 `switch (strategy)` 分流；新加策略要動好幾個地方（StrategyConfig 加欄位、/strategy/list 補 description、ParameterOptimizer 寫死 grid range、Lab MinCapital 字典）。

**做法**：`IStrategy` 介面改成 self-describing——`Description` / `Category` / `MinBars` / `MinCapitalUsdt` / `ParamSchema` 都是 default interface members。所有 13 個策略 override 這些 metadata。`IStrategyRegistry` 取代 raw dictionary，handler 從 registry 拿、`/strategy/list` 自動把 metadata 一起回傳。`StrategyConfig` 加 `Params: Dictionary<string, object>` 給新策略不必再擴 named field。

**對齊既有架構**：跟 §四 capability 自描述（route / approvalPolicy / category 都掛在 capability metadata）是同一招——**讓擴充點自己帶 metadata、不要靠外部 lookup table**。Commit `113c3b8`。

### 15.2 Distributed tracing on dispatch path

**發現**：BrokerCore 早就有 `IAuditService` 跟 hash chain 防篡改的 `audit_events` 表，BrokerService 16-step PEP 會寫 `EXECUTION_RECEIVED → ALLOWED → DISPATCHED → SUCCEEDED/FAILED` 四個事件。但 dashboard 直呼 dispatcher 那條輕量路徑（StrategyEndpoints / AutoTraderService 等）**繞過**了這套機制——`ApprovedRequest` 的 `WarnIfBypass` 機制就是 Benson 在標記這個 tech debt。

**做法**：把 audit emission 下沉到 `PoolDispatcher`——在派發前後寫 `DISPATCH_STARTED → SUCCEEDED/FAILED`。`request.TraceId` 沒設就自動產 `trc_xxx`、details JSON 帶 `worker_id` / `attempts` / `duration_ms`。Best-effort，audit 失敗不擋 dispatch。

`ScheduledBacktestService` 一個 backtest run（4 watches × 3 timeframes × 13 strategies ≈ 156 次跨 capability dispatch）整個串成同一個 `trace_id`、hash chain 連起來——是「fan-out tracing」的具體 demo。

**新端點**：`GET /api/v1/audit/traces`（最近 N 條 trace 摘要、預設過濾掉 ASP.NET HTTP middleware 的 W3C trace 噪音）、`GET /api/v1/audit/topology`（過去 N 分鐘 principal × capability 聚合）。Commits `7213e11`、`bf0fc92`。

**對齊**：跟 §三 「可審計」是同一招——任何進入受信邊界的動作都應該留 hash-chained 證據，不論進入路徑是 PEP 還是 dispatcher 直呼。

### 15.3 Tracing + Topology + Latency Dashboard

`index.html` 新增「追蹤」「拓撲」兩個 tab、跟既有容器/代理/能力分頁一致用 Vue 3 + Element Plus（曾經寫成獨立 vanilla 頁面、用戶看到後改成主 dashboard 並列、貫徹「平台一個入口」原則）。

- **追蹤**：列出最近 trace、點選後展開 Gantt 條（按 capability 分行、`STARTED → SUCCEEDED/FAILED` 配對畫綠/紅長條）、即時驗 hash chain integrity
- **拓撲**：sankey 桑基圖（principal → capability、線寬 ∝ 呼叫數、顏色 ∝ 成敗率）+ 4 個 KPI 卡 + 排行表
- **延遲**：拓撲分頁底部一張卡、每 capability 顯示 p50/p95/p99/max + 7-bucket 分布 inline histogram（log-scale）

零 d3 依賴、SVG + 純算 layout、demo 斷網也能跑。

**真實 production 數據**（過去 1h，從 VPS smoke test）：`strategy.signal` 41 次呼叫 / 100% 成功 / p50=4ms / p95=8ms / p99=25ms。Commits `7213e11`、`e8b944c`。

### 15.4 治理三件套：ACL + Approval + Per-user Override

把派發層從「只追蹤」推到「會擋」。三層治理閘道在 PoolDispatcher 入口依序檢查：

**(a) Role-based ACL**（`ICapabilityAclService`）

- `role_admin` / `system` → `*`（全開）
- `role_user` → `strategy.signal` / `quote.*` / `trading.account` / `trading.perpetual`
- **`trading.order` 故意不給 user**——多用戶 SaaS 階段所有手動下單必須走 AutoTrader（受 §14.10 那 4 條風控規則約束）
- Fail-open by design：empty role / unknown role → allow，不破壞既有 hardcode `PrincipalId="system"` 的內部呼叫
- 不通過 → emit `DISPATCH_DENIED` + 回 Fail

**(b) Approval workflow**（`IApprovalService` + `approval_requests` 表）

- `RequiresApproval(capability)` policy（預設只擋 `trading.order`）
- 第一次呼叫 → 寫 `status='pending'` + emit `DISPATCH_PENDING_APPROVAL` + 回 `Fail("approval_id=X, retry after approval")`
- 之後同 trace_id 重 dispatch → 看 status：`approved` 放行 / `rejected` 回 Fail
- system principal 略過 approval（內部背景任務 + AutoTrader 不會被擋）
- Admin endpoints：`GET /admin/approvals`、`POST /admin/approvals/{id}/approve|reject`

**(c) Per-principal override**（`principal_capability_overrides` 表）

- 「黃金客戶 user A 例外可以呼叫 trading.order」「降權 admin 暫時不能 trading.\*」
- 決策順序：principal `deny` > principal `allow` > role rule
- Admin endpoints：`GET/POST /admin/acl/overrides`

**Dashboard**：能力分頁底部多一張「ACL 角色規則」卡顯示 role × pattern；新分頁「待審」列待審核請求、admin 點按 ✓/✗ 即時決定。Commits `74ca713`、`b2f35c8`、`2d6dea4`、`25fa5ef`。

**對齊**：跟 §三 「broker 不執行、只裁決」是同一招——派發前的所有判斷都離開 hot path、做完寫 audit、不影響 worker 執行純度。ACL 的 fail-open 也跟 §三 「可重現」是同一招——不破壞既有路徑、新規則只在新進入點上生效。

### 15.5 Worker Health Score（綜合健康分數）

把分散在三個源頭的健康訊號（`IWorkerRegistry.LastHeartbeat` / `audit_events` 中 `DISPATCH_SUCCEEDED` 比例 / `IContainerManager.GetStatsAsync` 的 CPU+mem）壓成一個 0-100 分。

| 分量 | 權重 | 計算 |
|---|---|---|
| Heartbeat freshness | 30% | <30s→100、<60s→50、>60s→0 |
| Dispatch success rate | 40% | 過去 15 min `succeeded / total`（≥99%→100、95%→70、<95%→30） |
| Resource pressure | 30% | (CPU+mem)/2（<30%→100、<60%→70、<80%→40、≥80%→10） |

狀態切點：healthy ≥80 / degraded 50-79 / critical <50。沒接 ContainerManager 的環境會自動踢出 resource 分量、不懲罰。

**`GET /api/v1/health/score`**、容器分頁多一張彩色 KPI 卡（綠/橘/紅 border-left + 三色點顯示 healthy / degraded / critical 數）。

**真實 demo 數據**：剛部署完 trading-worker 因為先前的 reconnect flapping issue（broker 端週期性斷線、5-6 秒一次重連）→ dispatch 成功率 66.7% → score=64 (degraded)。其他 4 個 worker score=100。**Health Score 自動把人手動 grep log 才找到的 issue 標出來**——這就是「綜合指標」相對 raw metric 的價值。Commit `d631141`。

**對齊**：跟 §九 ScheduledDiagnosticsService 是同一招——把分散的健康訊號收斂成可決策的單一輸出，差別在於 Health Score 是即時計算、Diagnostics 是排程掃描，互補不重疊。

### 15.6 容錯：Auto-restart + Auto-scale

**Auto-restart**（`WorkerAutoRestartService`）：30s tick、看 `IContainerManager.ListManagedAsync` 找 state=Stopped/Failed 的 container、`StartWorkerAsync` 重啟、exponential backoff（1次→0s, 2→30s, 3→2min, 4→10min, 5→30min, ≥6→unrecoverable）。Container 持續 Running 5 min 後自動 reset 計數。

**Auto-scale**（`WorkerAutoScaleService`）：30s tick、依 worker_type 計 utilization（`Σactive / Σmax_concurrent`）、5-sample slide window：

- Scale-up：連 2 樣本 ≥80% 且 < `MaxContainersPerType` → `SpawnWorkerAsync`
- Scale-down：window 全 0 且 >1 instance → `StopWorkerAsync(oldest)`
- 60s 冷卻防 thrashing
- 一次只動一個容器、避免雪崩；下次 tick 看情況再動

只在 ContainerManager 真的啟用時才 register（compose stack 上 enabled、`NoOpContainerManager` skip）。Commits `4db135b`、`25fa5ef`。

**對齊**：跟 §三 「自我修復是基礎能力、不是優化」是同一招——容器層的健壯性不應該綁在人手介入；exponential backoff 跟 §五.3 worker SDK reconnect 退避策略也是同一招（client 跟 server 兩端用對稱的退避邏輯）。

### 15.7 整輪小結（給說詞用）

| §X 抽象主張 | §十五 哪一節提供實證 |
|---|---|
| §三 broker 是控制平面、不執行（且**有審計**） | 15.2（dispatcher 直呼路徑也納入 hash chain；任何進到 worker 的請求都留 trace） |
| §三 governance 是顯式的（policy / role / approval 都看得到） | 15.4（ACL 規則表 + 待審分頁 + 個別 override 都在 dashboard 上動） |
| §三 可觀測 / 可維運（治理層的事故能看到、不只是被記） | 15.3 + 15.5（Gantt + sankey + 延遲分布 + 健康分數一頁看完） |
| §五 control / execution plane 切分 | 15.6（auto-restart + auto-scale 都建在 IContainerManager 之上、不動 worker 介面） |
| §四 capability 自描述 | 15.1（IStrategy 介面 self-describing、registry 取代 lookup table） |

**設計哲學的進一步複利**：15.2 的 trace_id auto-generation 跟 §三 「政策資訊在 BrokerService 內物化」是同一招——必要的 metadata 在第一個合適的點補上、不要求 caller 配合（fail-open + auto-fix）。15.3 的 sankey 用純 SVG 而非 d3 跟 §九 dashboard 用 vanilla CSS 而非 framework 是同一個取向——demo 環境零依賴是可用性的一部分。15.4 的「fail-open + 顯式角色才強制」跟 §三 ApprovedRequest.WarnIfBypass 是同一招——對既有路徑友善、新規則不破壞向後相容、有偏離才警告。15.5 的 Health Score 用「沒資料的分量不懲罰」跟 §六 grant lookup 「沒有 grant 時 deny 但記原因」是同一招——「unknown ≠ failure」、明確區分「沒測量」跟「測量結果差」。15.6 的「只在 ContainerManager enabled 時 register」跟 §三 ToolSpecStatusChecker 是同一招——optional infrastructure 不該污染基本流程。

**這輪的可信度收尾**：12 個 commit、~2400 行新增、0 個 worker 介面變動、0 個既有 endpoint 行為變動（fail-open + additive only）。VPS production 上 broker 重啟 8 次都直接接回 4 個 worker 的 live 連線、AutoTrader 5 min cycle 沒中斷。
