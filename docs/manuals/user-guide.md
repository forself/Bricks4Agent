...
- LINE 訊息會先進入高階模型協調層，而不是直接進低階 agent
- 一般對話可直接回覆
- `query` 類請求可被判斷出來
- `production` 類請求會先建立 draft，要求使用者確認後才建立 task / plan
- 若是建立網站或其他專案型 production 任務，系統會先要求專案名稱，且同一使用者下不得重複

目前已確認的限制如下：

- 即時搜尋、天氣、票務、價格這類高階查詢工具仍在收斂中，現在不是所有 `query` 都會自動連到即時外部資料源
- 目前可確認的是「LINE -> broker 高階協調 -> draft / confirm 流程」已通，不代表所有下游產物都已自動完成
- 若要進行完整 governed / container 測試，仍需要 Podman 與對應本機環境

若你要的是「建立或修改外部產物」，系統目前的標準流程仍是：

1. 在 LINE 或高階對話中提出需求
2. 系統判斷為 production
3. 提供專案名稱或必要資訊
4. 回覆確認
5. 由 broker 建立 task / plan / handoff，交給下游機制
