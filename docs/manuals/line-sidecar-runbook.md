...
- provider: `anthropic` with model `claude-sonnet-4-6` when `ANTHROPIC_API_KEY` is present (preferred)

- otherwise the checked-in broker defaults apply (`ollama` / `qwen3.6:latest`); `openai-compatible` / `gpt-5.4-mini` is the last-resort fallback shape
