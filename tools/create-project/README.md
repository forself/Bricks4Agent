...
**機制 v2.1.0（2026-08-18）**：全腳本零依賴 Node（跨平台：Windows=junction、POSIX=symlink）；**tree 釘版**（`sync-lib.mjs --pin` → `b4a.lock.json` 入版控，tree 必須由目前 branch 可達，不依賴額外 tag 或 split commit）；**publish 強制**（lock 不符或 dirty 即 fail，`--allow-drift` 已停用）；`--no-junction` 建案自動釘版；**升級工具** `upgrade-project.mjs --dest <專案> [--dry-run]` 只更新機制腳本並跳過 customized 清單。

驗證要求：一般 clone、淺層 `--no-tags` clone 都必須能以 branch 內建 tree 完成 sync/publish；lock 竄改、tree 漂移、dirty 工作區與 drift bypass 均須 fail closed。
