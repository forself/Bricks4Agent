...
- **SVG banned — Canvas only** (legacy files ratcheted by `tools/scripts/svg-baseline.json`). Chart base `viz/CanvasChart.js`; theme reactivity `utils/theme-bus.js`; color fallbacks only via theme-bus `FALLBACK_PAINT` (no loose hex).
