...
public class InProcessDispatcher : IExecutionDispatcher
{
    ...
    private async Task<List<object>> SearchGoogleWithFallbackAsync(string query, int limit, string locale, string safeMode)
    {
        ...
        return await SearchGoogleAsync(query, limit, locale, safeMode);
    }
        ...
        Func<TdxApiService, string, CancellationToken, Task<object?>> tdxQuery)
    {
        ...
        try
        {
            ...
            var query = args.TryGetProperty("query", out var queryNode) ? queryNode.GetString() ?? "" : "";
            if (string.IsNullOrWhiteSpace(query))
                return null;

            var result = await tdxQuery(_tdxApiService, query, CancellationToken.None);
            if (result == null)
                return null;
