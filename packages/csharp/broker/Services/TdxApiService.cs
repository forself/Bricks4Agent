...
public sealed class TdxApiService
{
    ...
    /// <summary>台鐵今日時刻表（起訖站）</summary>
    public Task<JsonDocument?> GetTraDailyTimetableAsync(
        string originStation, string destStation, CancellationToken ct = default)
    {
        var path = $"v2/Rail/TRA/DailyTimetable/OD/{Uri.EscapeDataString(originStation)}/to/{Uri.EscapeDataString(destStation)}/today?$format=JSON";
    ...
    /// <summary>高鐵今日時刻表（起訖站）</summary>
    public Task<JsonDocument?> GetThsrDailyTimetableAsync(
        string originStation, string destStation, CancellationToken ct = default)
    {
        var path = $"v2/Rail/THSR/DailyTimetable/OD/{Uri.EscapeDataString(originStation)}/to/{Uri.EscapeDataString(destStation)}/today?$format=JSON";
        return GetAsync(path, ct);
    }
