...
public class PlanEngine : IPlanEngine
{
    ...
    public Plan SubmitAndExecute(string planId, string principalId,
                                  string sessionId, string traceId)
    {
        ...
        while (iteration < maxIterations)
        {
            ...
            {
                ExecuteNode(node, plan, principalId, sessionId, traceId);
            }
        }
    ...
    private void ExecuteNode(PlanNode node, Plan plan, string principalId,
                              string sessionId, string traceId)
    {
        // Mark running
        ...
        try
        {
            ...
            // Step B: 透過 BrokerService PEP 16 步裁決 + 執行
            var idempotencyKey = $"plan_{plan.PlanId}_node_{node.NodeId}_attempt_{node.RetryCount}";
            var execRequest = _brokerService.SubmitExecutionRequest(
                principalId,
                plan.TaskId,
                sessionId,
                node.CapabilityId,
    ...
    private string InjectDataFlowContext(PlanNode node, string principalId)
    {
        ...
        try
        {
            ...
            payload["_upstream"] = JsonDocument.Parse(upstreamJson).RootElement.Clone();
