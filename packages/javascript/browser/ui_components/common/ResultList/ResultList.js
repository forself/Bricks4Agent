...
export class ResultList {
    ...
    _injectStyles() {
        if (document.getElementById('resultlist-component-styles')) return;
        const s = document.createElement('style');
        s.id = 'resultlist-component-styles';
        s.textContent = `
            .cl-resultlist { display: flex; flex-direction: column; gap: 12px; }
            .cl-resultlist-item { padding: 10px 12px; border: 1px solid var(--cl-border); border-radius: var(--cl-radius-md); }
            .cl-resultlist-item .cl-link { font-weight: 600; }
            .cl-resultlist-meta { display: flex; gap: 6px; align-items: center; margin-top: 4px; }
            .cl-resultlist-empty { color: var(--cl-text-secondary); padding: 16px; text-align: center; }
        `;
