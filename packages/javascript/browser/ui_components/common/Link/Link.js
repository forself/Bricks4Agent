...
export class Link {
    ...
    constructor(options = {}) {
        ...
        // :hover 偽類 → CSSOM 事件切換(CSP 合規,同 DownloadButton 模式)
        this._onMouseEnter = () => {
            this._hovered = true;
            const { content } = this.snapshot();
            if (this.element && !content.disabled) this.element.style.textDecoration = 'underline';
        };
        this._onMouseLeave = () => {
            this._hovered = false;
            if (this.element) this.element.style.textDecoration = 'none';
        };
    ...
    _composeCssText(content, visibility) {
        const decl = [
            ...
            `text-decoration: ${this._hovered && !content.disabled ? 'underline' : 'none'}`,
            'cursor: pointer'
        ];
        if (content.scope === Link.SCOPES.NONE) {
            decl.push('color: var(--cl-text-primary)', 'cursor: default');
        }
        if (content.disabled) {
            decl.push('color: var(--cl-text-disabled)', 'cursor: not-allowed', 'pointer-events: none');
        }
