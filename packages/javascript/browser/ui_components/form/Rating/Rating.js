...
export class Rating {
    ...
    _create() {
        ...
        this.element.style.cssText = 'display: inline-flex; gap: 2px; color: var(--cl-warning); line-height: 1;';
        const max = Math.max(1, parseInt(this.options.max, 10) || 5);
        for (let i = 1; i <= max; i++) {
            const star = document.createElement('span');
            star.className = 'cl-rating-star';
            star.dataset.index = String(i);
            star.style.cssText = 'width: 18px; height: 18px;' + (this.options.readonly ? '' : ' cursor: pointer;');
            const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
            svg.setAttribute('viewBox', '0 0 24 24');
            svg.style.cssText = 'width: 18px; height: 18px; display: block;';
            ...
            if (!this.options.readonly) {
                ...
                // hover 預覽::hover 規則改為事件切換(CSP 合規)
                star.addEventListener('mouseenter', () => this._paintStars(i));
            }
            this.element.appendChild(star);
            this._stars.push({ index: i, el: star, path });
        }
        if (!this.options.readonly) {
            this.element.addEventListener('mouseleave', () => this._paintStars(this.getValue()));
        }
    }

    _paintStars(value) {
        for (const { index, path } of this._stars) {
            path.setAttribute('fill', index <= value ? 'currentColor' : 'none');
    ...
    _applyState() {
        ...
        // display 由 cssText 承載 inline-flex,隱藏/顯示需明確還原(不可設 '' 退回 UA 預設)
