...
export class PageGenerator {
    ...
    constructor(options = {}) {
        ...
        this.toolRendererImportPath = options.toolRendererImportPath || '@component-library/page-generator/DynamicToolRenderer.js';
    }
    ...
    generate(definition) {
        ...
        if (definition.type === PageTypes.TOOL) {
            return {
                code: this._generateToolCode(definition),
                errors: []
            };
        }
    ...
    _generateToolCode(definition) {
        const serializedDefinition = JSON.stringify(definition, null, 4)
            .replace(/</g, '\\u003c')
            .replace(/\u2028/g, '\\u2028')
            .replace(/\u2029/g, '\\u2029');
        ...
        return `/**
 * ${className} - declarative tool page
 * @generated by PageGenerator
 */

import { DynamicToolRenderer } from ${JSON.stringify(this.toolRendererImportPath)};

export const definition = ${serializedDefinition};

export class ${className} {
    constructor(options = {}) {
        this.options = {
            commandRegistry: options.commandRegistry || options.commands || new Map(),
            state: options.state || {},
            factory: options.factory || null,
            controlRegistry: options.controlRegistry || null
        };
        this.renderer = null;
    }

    async init() {
        this.renderer = new DynamicToolRenderer({
            definition,
    ...
    mount(container) {
        if (!this.renderer) throw new Error('Call await init() before mount().');
        this.renderer.mount(container);
        return this;
    }

    getComponent(id) {
        return this.renderer?.getComponent(id) || null;
    }

    getHost(id) {
        return this.renderer?.getHost(id) || null;
    }

    setState(path, value) {
        if (!this.renderer) throw new Error('Call await init() before setState().');
        this.renderer.setState(path, value);
        return this;
    }

    get controlRecords() {
        return this.renderer?.controlRecords || null;
    }
...
`;
        ...
        // M-2 修復：解析語意日期預設值（'today' → runtime 當前日期）
        for (const field of definition.fields || []) {
            if (field.type === FieldTypes.DATE && field.default === 'today') {
                code += `
        // 解析語意預設值: ${field.name} = 'today' → 當前日期
        if (this._data.form.${field.name} === 'today') {
            this._data.form.${field.name} = new Date().toISOString().split('T')[0];
        }
`;
            }
        }
